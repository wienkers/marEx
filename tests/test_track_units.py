...
class TestWrappedEuclidianParallel:
    """Test wrapped_euclidian_parallel function for distance calculations."""
    ...
    def test_wrapped_euclidian_basic(self):
        ...
        result = track.wrapped_euclidian_parallel(mask, centroids, Nx=10)
    ...
    def test_wrapped_euclidian_edge_cases(self):
        ...
        result = track.wrapped_euclidian_parallel(mask, centroids, Nx=5)
...
class TestCalculateWrappedDistance:
    """Test calculate_wrapped_distance function for single distance calculations."""
    ...
    def test_calculate_wrapped_distance_x_wrapping(self):
...
class TestDistanceCalculationValidation:
    ...
    def test_wrapped_distance_minimum_value(self):
        ...
        dist = track.calculate_wrapped_distance(0, 9, 0, 0, nx=10, half_nx=5.0)
        assert np.isclose(dist, 1.0, atol=1e-10)

        # Point at (0, 6) should be distance 4 from (0, 0) (not wrapped)
        dist = track.calculate_wrapped_distance(0, 6, 0, 0, nx=10, half_nx=5.0)
        assert np.isclose(dist, 4.0, atol=1e-10)

        # Point at (0, 4) should be distance 4 from (0, 0) (not wrapped)
        dist = track.calculate_wrapped_distance(0, 4, 0, 0, nx=10, half_nx=5.0)
        assert np.isclose(dist, 4.0, atol=1e-10)
